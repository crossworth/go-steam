...
func TestBinaryEncoder_Encode(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			Expected: nil,
			Err:      "cannot encode nodes of type End",
		},
		{
			Subject:  kv.NewKeyValue(kv.TypeWideString, "", "", nil),
			Expected: nil,
			Err:      "cannot encode nodes of type WideString",
		},
		{
			Subject:  kv.NewKeyValue(kv.TypeInvalid, "", "", nil),
			Expected: nil,
			Err:      "cannot encode nodes of type Invalid",
		},
		{
			Subject:  kv.NewKeyValueString("K", "S", nil),
			Expected: []byte{kv.TypeString.Byte(), 'K', 0x00, 'S', 0x00},
		},
		{
		...
		},
		{
			...
			Expected: []byte{
				...
				'K', 0x00,
				0x01, 's', 0x00, 'S', 0x00,
				kv.TypeEnd.Byte(), kv.TypeEnd.Byte(),
			},
		},
		{
