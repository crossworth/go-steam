package steam

import (
	"crypto/rsa"

	"github.com/13k/go-steam-resources/steamlang"
	"github.com/13k/go-steam/cryptoutil"
)

var (
	publicKeysData = map[steamlang.EUniverse][]byte{
		steamlang.EUniverse_Public: {
			,
		},
		steamlang.EUniverse_Beta: {
			,
		},
		steamlang.EUniverse_Internal: {
			,
		},
		steamlang.EUniverse_Dev: {
			,
		},
	}

	publicKeys = map[steamlang.EUniverse]*rsa.PublicKey{
		steamlang.EUniverse_Public:   cryptoutil.MustParseASN1RSAPublicKey(publicKeysData[steamlang.EUniverse_Public]),
		steamlang.EUniverse_Beta:     cryptoutil.MustParseASN1RSAPublicKey(publicKeysData[steamlang.EUniverse_Beta]),
		steamlang.EUniverse_Internal: cryptoutil.MustParseASN1RSAPublicKey(publicKeysData[steamlang.EUniverse_Internal]),
		steamlang.EUniverse_Dev:      cryptoutil.MustParseASN1RSAPublicKey(publicKeysData[steamlang.EUniverse_Dev]),
	}
)

func GetPublicKey(universe steamlang.EUniverse) *rsa.PublicKey {
	return publicKeys[universe]
}
